...
async function seedDatabase() {
  try {
    ...
    if (settingsCount === 0) {
      ...
      // Fix existing settings if they are still using the defaults
      await db.prepare("UPDATE settings SET value = '/logo.svg' WHERE key = 'logo_url' AND value = '/logo.png'").run();
      await db.prepare("UPDATE settings SET value = 'https://www.w3schools.com/html/movie.mp4' WHERE key = 'hero_video' AND (value = 'https://player.vimeo.com/external/434045526.sd.mp4?s=c27d23d8c1ad2125e98583fb24285e683f491176&profile_id=165&oauth2_token_id=57447761' OR value IS NULL)").run();
      console.log('🔄 Existing logo_url and hero_video defaults updated in database settings.');
    }
